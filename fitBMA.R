#' Perform Bayesian Model Averaging (BMA) over entire model space
#' 
#' Perfroms BMA over entire model space and produces relevant output.
#' 
#' @param M a matrix of observations for the covariates.
#' @param Y a numeric vector whose length is equal to the number of rows as \code{M}.
#' @param gprior a user defined prior.
#' 
#' @return An object of class list with elements
#'  \item{Coeffs}{A matrix of coefficient estimates generated by the regressions of all possible variable combinations.}
#'  \item{R2s}{A vector of R-squared values generated by the regressions of all possible variable combinations.}
...
#covariates and then peform the linear regressions. In order to remove the intercept term, we subtract
#1 in the linear model. From here, we extract the R2 values and the coefficients and store them as
#R2s and coefs.
    for(i in 1:ncol(M)){ 
...
#converted into a matrix. To calculate this, we sum the product of the posterior model odd and 
